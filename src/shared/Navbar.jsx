    ...
    <>
      <li>
        <NavLink
          ...
          className={({ isActive }) =>
            isActive
              ? "custom-text-secondary font-medium"
              : "custom-text-primary hover:custom-text-secondary transition-colors"
          }
        >
          Home
        </NavLink>
    ...
    <div className="navbar custom-bg-primary sticky top-0 z-50 px-4 lg:px-8 xl:px-32 mx-auto backdrop-blur-xl shadow-lg border-b border-slate-200 dark:border-slate-700 transition-all duration-300">
      ...
      <div className="navbar-end gap-3">
        <button onClick={handleThemeChange} className="btn btn-ghost btn-circle">
          {theme === "light" ? (
            <Sun size={20} className="custom-text-secondary" strokeWidth={1.5} />
          ) : (
            <Moon size={20} className="custom-text-secondary" strokeWidth={1.5} />
          )}
        </button>
        ...
        {user && (
          ...
          >
            <div className="w-10 rounded-full ring-1 ring-[var(--bg-secondary)] transition-all duration-300">
              <img
        ...
        {user ? (
          ...
          <div className="flex gap-2">
            <Link
              to="/login"
              className="px-4 py-2 border border-[var(--text-secondary)] custom-text-secondary rounded-lg hover:custom-bg-secondary transition-all duration-300 outfit text-sm"
            >
              Login
            </Link>
            <Link
              to="/signup"
              className="px-4 py-2 bg-teal-600 hover:bg-teal-700 dark:bg-teal-700 dark:hover:bg-teal-600 text-white rounded-lg transition-all duration-300 outfit text-sm"
            >
              Signup
            </Link>
          </div>
